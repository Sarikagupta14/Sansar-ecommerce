import { Box, Button, Flex, FormControl, FormLabel, Heading, Image, Input, Text, useBreakpoint, useBreakpointValue, useToast } from '@chakra-ui/react'
...
if (!card.trim()) {
  ...
  return (
    <Flex direction={{base:'column', lg:'row'}}>
      ...
      {/* Cart items */}
    <Box w={{base:'90%',lg:'48%'}} ml={'2%'}>
    <Heading color={'teal'} letterSpacing={'2px'} ml={'100px'}> Cart Items</Heading>
    {
    update && update.map((item)=>{
        return(
            ...
            <Text color={'teal'} fontSize={'18px'}>{item.brand}</Text>
   ...
   {/* User details */}
   {smallScreen ? (
<Box  boxShadow={'10px 10px 10px 7px black'} borderTopLeftRadius={'30px'}  ml={'55%'} mt={'40px'} w={'40%'} h={'580px'}  border={'2px solid black'} position={{lg:'fixed'}}>
...
<Box  m={'auto'} fontSize={'20px'} borderRadius={'15px'} mt={'40px'} w={'200px'} > 
   <Button onClick={handleClick}  color={'white'} border={'1px solid black'} bg={'teal'} fontWeight={'600'}>Proceed for payment</Button>
</Box>
</Box>
</Box>
): 
